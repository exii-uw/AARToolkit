...
namespace PlaneFinding
{
    ...
    bool FillVertexCurvatures(_Out_ vector<PerVertexData> *vertexData, _In_ HalfEdgeMesh *halfEdge, _In_ XMFLOAT3 *normals, _In_ UINT32 vertCount)
    {
        for (UINT32 i = 0; i < vertCount; ++i)
        {
            ...
            if (numNeighbors == 0)
            {
                // we expect no floating vertices without neighbors
				numNeighbors = 1;
            }
        ...
        _In_ float snapToGravityThreshold)
    {
        ...
        for (int i = 0; i < numMeshes; ++i)
        {
            ...
            // First we calculate the curvature for every vertex
			if (!FillVertexCurvatures(&vertexData, &halfEdge, normals, vertCount)) continue;
